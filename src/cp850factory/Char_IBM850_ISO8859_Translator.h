// -*- C++ -*-

//=============================================================================
/**
 *  @file    Char_IBM850_ISO8859_Translator.h
 *
 *  Declares the arrays required to convert between ISO8859 (aka
 *  Latin/1) and IBM850 (aka EBCDIC).
 *
 *  @author Jim Rogers (jrogers@viasoft.com)
 */
//=============================================================================


#ifndef TAO_CHAR_IBM850_ISO8859_TRANSLATOR_H
#define TAO_CHAR_IBM850_ISO8859_TRANSLATOR_H
#include /**/ "ace/pre.h"

#include "ace/config-all.h"

#if !defined (ACE_LACKS_PRAGMA_ONCE)
# pragma once
#endif /* ACE_LACKS_PRAGMA_ONCE */

#include "ace/CDR_Stream.h"
#include "IBM850_ISO8859_export.h"

extern IBM850_ISO8859_Export char to_IBM850[257];
extern IBM850_ISO8859_Export char from_IBM850[257];

// ****************************************************************

/**
 * @class ACE_Char_IBM850_ISO8859
 *
 * @brief Codeset translation specialization.
 *
 * This class performs the codeset translation:
 *   - Native:        IBM_1047 (i.e. EBCDIC)
 *   - Stream:        ISO-8859 (i.e. Latin/1)
 */
class IBM850_ISO8859_Export IBM850_ISO8859 : public ACE_Char_Codeset_Translator
{
public:
  /// A do nothing constructor.
  IBM850_ISO8859 (void);

  /// Virtual destruction
  virtual ~IBM850_ISO8859 (void);

  // = Documented in $ACE_ROOT/ace/CDR_Stream.h
  virtual ACE_CDR::Boolean read_char (ACE_InputCDR &, ACE_CDR::Char &);
  virtual ACE_CDR::Boolean read_string (ACE_InputCDR &, ACE_CDR::Char *&);
  virtual ACE_CDR::Boolean read_char_array (ACE_InputCDR &,
                                            ACE_CDR::Char *,
                                            ACE_CDR::ULong);
  virtual ACE_CDR::Boolean write_char (ACE_OutputCDR &, ACE_CDR::Char);
  virtual ACE_CDR::Boolean write_string (ACE_OutputCDR &,
                                         ACE_CDR::ULong,
                                         const ACE_CDR::Char *);
  virtual ACE_CDR::Boolean write_char_array (ACE_OutputCDR &,
                                             const ACE_CDR::Char *,
                                             ACE_CDR::ULong);
  virtual ACE_CDR::ULong ncs () {return 0x10021352LL;}
  virtual ACE_CDR::ULong tcs () {return 0x00010001LL;}
};

#include /**/ "ace/post.h"
#endif /* TAO_CHAR_IBM850_ISO8859_TRANSLATOR_H */
